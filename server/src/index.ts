...
async function start(): Promise<void> {
  ...
  app.listen(PORT, HOST, () => {
    ...
    console.log(`GraphQL endpoint available at http://${HOST}:${PORT}/graphql`);
    if (HOST === 'localhost') {
      // Android emulators' method to access the host machine (and thus the GraphQL endpoint)
      console.log(`\nAndroid emulators can access the GraphQL endpoint at http://10.0.2.2:${PORT}/graphql`);
    }
  });
}
